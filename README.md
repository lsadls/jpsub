...
- **标为未译（↺）**：某句 AI 翻得烂时，点时间栏行末的 **↺**，该句标回未译并清掉缓存里的旧译文；
- **续翻 / 重翻 / 硬核**：操作区的「续翻」只重翻标过未译的句子（已译的跳过省 token）；「重翻」全部重翻（等价 `--force`）；勾选「硬核」则把每条原文按「。」拆成单句逐句翻译后拼回，单个句子翻坏不会连累整条（请求次数会明显变多）。翻译进度实时显示在消息区，完成后自动刷新页面；
...
> **硬核模式 `--hard`**：`translate` / `text` 加此参数后,每条原文按「。」拆成单句逐句翻译再按原序拼回——单句被审查/翻坏只回退那一句,不会毁掉整条。请求次数明显变多,适合对付经常翻坏长段的素材。

> **翻译任意文本文件**：`jpsub text 文件.txt` 逐行发给 AI 翻译,输出 `文件.zh.txt`（`-o` 可指定输出）。支持 `--comment` 背景引导、`--glossary` 名词对照表、`--hard` 拆句模式。
