...
工作目录(如 output\sm114514.jpsub)下的 `translate-in.txt`(日文原文)和 `translate-out.txt`(中文译文),每行格式为 `起-止<TAB>文本`,时间轴为军方时间(无冒号),对应字幕显示的起止时间:
...
- **效果**三选一：`模糊`（可调强度）、`纯色`（取色器选色）、`图片`（点「选图片…」上传，图片会存到视频同目录）；
- 点 **保存** 写入 `masks.txt`（与视频同目录），点 **应用打码** 直接输出 `sm43168834.masked.mp4`。
